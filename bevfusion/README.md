# BEVFusion
This is a codebase based on BEVfusion [http://bevfusion.mit.edu/](https://arxiv.org/abs/2205.13542).
...
## personal notes
1. Refer to dockerfile for list of commands to run for installing dependencies
2. If using sc compute resources:
- run command to check gcc version if newer versions of gcc required ```module avail``` 
- run command to load required module ```module load <module_name>```
- run command to load cuda for nvidia gpu ```module load <cuda module name>``` 
- run ```conda install ‐c nvidia cuda‐toolkit=<version>```