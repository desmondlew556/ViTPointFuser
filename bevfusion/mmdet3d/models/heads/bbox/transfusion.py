...
class TransFusionHead(nn.Module):
    ...
    def filter_bboxes(self,predicted_bboxes,thresholds,min_detections,feature_maps,point_cloud_range):
        """
        Get bboxes that have scores above threshold.
        Args:
            predicted_bboxes (list[list[dict]]): Decoded bbox, scores and labels for each layer & each batch
            thresholds (list): Range between 0-1. thresholds used to select bboxes. If there are less than min_detections bboxes predicted, use the next threshold. 
            min_detections (int): min number of detections required for threshold, otherwise keep lowering thresholds according to thresholds
            feature_maps (Batch size x Channels x Y_dim x X_dim): feature maps from which to extract corresponding detected object features
            point_cloud_range (list of shape (6,)): [-x -y -z x y z]
        Return:
            list[dict]: 
                keys:
                    bboxes':Full set of bboxes
                    'scores': Full set of scores
                    'labels': Full set of labels
                    'sample_data': Each row contains data for a single detection with scores above threshold
                    'feature_map: Full feature map used
                Decoded bbox that are above the threshold, scores and labels for each layer & each batch
        """
        data = []
        for sample_idx in range(len(predicted_bboxes)):
            bboxes_data = predicted_bboxes[sample_idx]

            assert len(thresholds)>1 and len(thresholds)<10, "Current thresholds limits set to 10 thresholds"
            # get only boxes with confidence above a threshold
            threshold = thresholds[0]
            abv_threshold_indices = bboxes_data[1]>=threshold

            # keep lowering threshold in thresholds list until at least min_detections predicted bboxes are obtained.
            num_detections = bboxes_data[1][abv_threshold_indices].shape[0]
            threshold_idx=1
            while num_detections<min_detections and threshold_idx<len(thresholds):
                threshold = thresholds[threshold_idx]
                abv_threshold_indices = bboxes_data[1]>=threshold
                num_detections = bboxes_data[1][abv_threshold_indices].shape[0]
                threshold_idx+=1
            
            bboxes = bboxes_data[0][abv_threshold_indices]
            scores = bboxes_data[1][abv_threshold_indices]
            labels = bboxes_data[2][abv_threshold_indices]

            # skip sample if no detections
            print("number valid detections: %s, threshold used %s"%(scores.shape[0],threshold))
            if (scores.shape[0]<1):
                data.append(None)
                continue
            # get corresponding feature map region of 3m around center of detected object
            BOX_DIM = 3  # real world box dimensions 
            point_cloud_range = np.array(point_cloud_range)
            # original point cloud range is -x,-y,-z,x,y,z change to z,y,x 
            point_cloud_size = (point_cloud_range[3:5]-point_cloud_range[:2])[::-1] # get range for x and y dims. Reverse shape to get y as first index

            feature_maps = feature_maps[0].cpu()
            scale_factor = feature_maps.shape[2:]/point_cloud_size # Feature map is Batch_size x Channels x Y_dim x X_dim
            y_width = int(np.ceil(BOX_DIM/2*scale_factor)[0]) 
            x_width = int(np.ceil(BOX_DIM/2*scale_factor)[1])
            
            # detections_feature_maps = np.zeros((len(bboxes.bev),feature_maps.shape[1]*4*y_width*x_width))

            # for idx in range(bboxes.bev.shape[0]):
            #     x,y,_,_,_ = bboxes.bev[idx]
            #     # use this
            #     transformed_y = int(np.round((y.cpu()-point_cloud_range[1])*scale_factor[0])) # transform coordinates to index. 0,0 starts from top left. y increases downwards, and x increases rightwards.
            #     transformed_x = int(np.round((x.cpu()-point_cloud_range[0])*scale_factor[1]))

            #     transformed_y = int(np.round(-y.cpu()*scale_factor[0])+feature_maps.shape[2]/2) # transform coordinates to index. 0,0 starts from top left. y increases downwards, and x increases rightwards.
            #     transformed_x = int(np.round(x.cpu()*scale_factor[1])+feature_maps.shape[3]/2)
                
            #     fm_tmp = feature_maps[sample_idx,:,max(0,transformed_y-y_width):min(transformed_y+y_width,feature_maps.shape[2]),max(transformed_x-x_width,0):min(transformed_x+x_width,feature_maps.shape[3])]
            ...
            #                 fm_tmp,
            #             ),axis = 1)
            #     elif (transformed_y+y_width-1>feature_maps.shape[2]-1):
            #         # pad zeros to bottom
            #         fm_tmp = torch.cat((
            #                 fm_tmp,
            #                 torch.zeros((fm_tmp.shape[0],transformed_y+y_width-feature_maps.shape[2],fm_tmp.shape[2])),
            #             ),axis = 1)
            ...
            #                 fm_tmp,
            #             ),axis = 2)
            #     elif (transformed_x+x_width-1>feature_maps.shape[3]-1):
            #         # pad zeros to the right
            #         fm_tmp = torch.cat((
            #                 fm_tmp,
            #                 torch.zeros((fm_tmp.shape[0],fm_tmp.shape[1],transformed_x+x_width-feature_maps.shape[3])),
            #             ),axis = 2)
                    
            #     detections_feature_maps[idx,:]=fm_tmp.flatten()

            # Each row: x,y,x_width,y_width,yaw,label,confidence_score
            sample_data = np.concatenate(
                (
                np.array(bboxes.bev.cpu()),
                np.array(labels.reshape(len(labels),-1).cpu()),
                np.array(scores.reshape(len(labels),-1).cpu()),
                ),
                axis = 1,
            )
            sample_data_dict = {
                # 'bboxes':np.array(bboxes_data[0].tensor.cpu()),
                # 'scores':np.array(bboxes_data[1].cpu()),
                # 'labels':np.array(bboxes_data[2].cpu()),
                'sample_data':sample_data,
                # Shape is Channels x Y_dims x X_dims
                'feature_map':feature_maps[0],
                # channel width of N layers, delimiter, scale_factor for y, scale_factor for x, point cloud range of -y, point cloud range of -x,y_width, and x_width 
                'metas':np.array([256,256,-1,scale_factor[0],scale_factor[1],point_cloud_range[1],point_cloud_range[0],y_width,x_width])
                
            }
            data.append(sample_data_dict)
        return data
